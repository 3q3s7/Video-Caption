...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, ix):
        ...
        fc_feat.append(np.load(os.path.join(self.feats_dir, 'G_00%i.npy' % (ix+100))))
        fc_feat = np.concatenate(fc_feat, axis=1)
        if self.with_c3d == 1:
            c3d_feat = np.load(os.path.join(self.c3d_feats_dir, 'G_00%i.npy'%(ix+100)))
        ...
        captions = self.captions['G_00%i'%(ix+100)]['final_captions']
